...
data class SearchParams(
    ...
    fun trackableId() =
        "sortBy=$sortBy;sortDir=$sortDir;description=$description;skillsPossessed=$skillsPossessed;skillsSought=$skillsSought;tools=$tools;languages=$languages;availability=$availability;timezones=$timezones;page=$page;"
    ...
    fun query(): Query {
        ...
        var hasAnyFieldBeenAdded = false

        /** STEP 1: FILTER ON REQUIRED FIELDS TO ENSURE ALL RESULTS CONTAIN ALL ELEMENTS */
        skillsPossessed?.let {
            hasAnyFieldBeenAdded = true
            builder.filter(matchQuery {
                field(PostItem::skillsPossessed.name);
                query(skillsPossessed.toFieldValue())
            })
        }
        skillsSought?.let {
            hasAnyFieldBeenAdded = true
            builder.filter(matchQuery {
                field(PostItem::skillsSought.name);
                query(skillsSought.toFieldValue())
            })
        }
        languages?.let {
            hasAnyFieldBeenAdded = true
            builder.filter(matchQuery {
                field(PostItem::languages.name);
                query(languages.toFieldValue())
            })
        }
        tools?.let {
            hasAnyFieldBeenAdded = true
            builder.filter(matchQuery {
                field(PostItem::preferredTools.name);
                query(tools.toFieldValue())
            })
        }
        ...
        if (!hasAnyFieldBeenAdded) {
            return MatchAllQuery.builder().build().toQuery()
        }
